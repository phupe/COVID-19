                          ...
                          scale = 1.3)
{
    ...
    daily.cumulative.death$death[ind] <-  daily.cumulative.death$death[ind] * scale
    daily.cumulative.death$death[ind] <-  as.integer(floor(daily.cumulative.death$death[ind]))
                        ...
                        max.date.pred = as.Date("05/01/20", format = "%m/%d/%y"))
{
    ...
    for (country in countries.of.interest)
    {
        ...
        {
            stop(paste(
                "ERROR: population is not available for country = ",
                country
            ))
        } else
        {
