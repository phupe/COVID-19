...
fitBrody <- function(death.country)
{
    ...
    death.country$newtime <- death.country$time - min(death.country$time)
    ...
    prediction.value <- exp(Alogis * (1 - exp(-scal * (death.country$newtime - xmid))^3))
                            ...
                            death.country = NULL)
{
    ...
    l.u.int <-
        data.frame(
                   lower = l.u.int.values$summary$Prop.2.5,
